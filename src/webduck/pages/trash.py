...
def register():
    ...
    def trash_page():
        ...
        def ask_overwrite(result: dict, trash_name: str):
            with ui.dialog() as dlg, ui.card().classes("items-center gap-4").style("background: #1E1E1E; border-radius: 12px; padding: 24px 32px;"):
                if result["type"] == "database":
                    ui.label(_("restore_conflict_db").format(result["database"], result["project"])).style(f"color: {TEXT_SOFT}")
                else:
                    ui.label(_("restore_conflict_project").format(result["project"])).style(f"color: {TEXT_SOFT}")
                ui.label(_("restore_conflict_hint")).classes("text-caption").style("color: #999; max-width: 420px; text-align: center;")
        ...
        def restore_entry(entry: dict):
            async def do_restore():
                ...
                if entry["type"] == "database":
                    ...
                    result = await asyncio.to_thread(ctx.storage.restore_project, trash_name)
                if result.get("success"):
                    ...
                    ui.notify(result.get("error") or _("error"), type="negative")
        ...
        with ui.card().classes("w-full").style("margin-top: 4px"):
            with ui.row().classes("w-full items-center gap-4"):
                ui.label(_("trash_title")).classes("text-h5").style(f"color: {YELLOW_LIGHT}")
                ui.space()
                ...
                async def empty_trash():
                    with (
                        ...
                        ui.label(_("confirm_empty_trash")).style(f"color: {TEXT_SOFT}")
                        with ui.row().classes("gap-2"):
                            ui.button(
                                _("cancel"),
                                on_click=dlg.close,
                ...
                ).props("outline color=red").tooltip(_("empty_trash")).props("tooltip-position=top")
            ...
            with ui.card().classes("w-full q-mt-sm"):
                with ui.row().classes("w-full items-center gap-2"):
                    if entry["type"] == "project":
                        ...
                        ui.label(entry["project"]).classes("text-h6").style(f"color: {YELLOW_LIGHT}")
                        ...
                        ui.label(f"{_('project')}: {entry['project']}").classes("text-caption").style("color: #888;")
                    ui.space()
                    ui.label(_fmt_bytes(entry["size"])).classes("text-caption").style("color: #888; font-size: 0.75em;")
                    ui.label(f"{_('deleted_at')}: {deleted_at[:16]}").classes("text-caption").style("color: #888; font-size: 0.75em;")
                    ui.button(
                ...
                if entry["type"] == "project" and entry.get("databases"):
                    with ui.column().classes("q-pl-2 gap-2"):
                        ui.label(_("databases")).classes("text-caption").style("color: #888;")
                        for db in entry["databases"]:
                            with ui.row().classes("items-center gap-2 q-pl-2"):
                                ui.html(
                                ...
                                ui.label(db["name"]).classes("text-body2").style("color: #E0E0E0;")
                                ui.space()
                                ui.label(_fmt_bytes(db["size"])).classes("text-caption").style("color: #888; font-size: 0.75em;")
